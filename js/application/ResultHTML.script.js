...
// FileName ResultHTML.script.js: Result Javascript file for Cordova project
...
function onBackKeyDown() {
	...
	var resultScript = ResultScript.getInstance();
	resultScript.onClickListBackButton();
...
function onDeviceReady() {
	...
	$(".main-container").prepend( resultSearchBar );
	$(".main-container").prepend( resultHeader );

	$("#activity_title").text( "Result" );
	...
	resultScript.bindFormEventHandlers();

	// Hide Banner Ads
	// hideBanner();

	resultScript.onListDocumentReady();
}
...
function onFileUploadSuccess() {
	...
	if( mFile != null && mUploadedFiles.length == 0 ) {
		...
		resultScript.uploadDocuments();
	}
	else {

		resultScript.onConfirmNetworkSaveData();
	}
}
...
var resultSearchBar = `	<!-- Search Bar-->
<div class="page-container" id="search_bar">
	<!-- Search Input -->
	<div class="form-group" style="margin: -8px;">
		<div class="input-group mb-4">
			<input class="form-control" placeholder="Search results..." id="search" type="text">
			<!-- Search Button: sits inside the same input-group as
			     #search, right next to the box, on the same line -
			     matches Student List's #search_icon. Click handler
			     (searchList()) is already bound in
			     Result.script.js/onListDocumentReady(); this button
			     was missing from the markup so that handler had
			     nothing to bind to. -->
	...
	<!-- STANDARDIZED TOOLBAR (identical markup on Student/Category/
	     ...
	     click handlers in Result.script.js/onListDocumentReady()
	     keep working. -->
	...
	<div class="modal-dialog modal-danger modal-dialog-centered modal-10" role="document">
		<div class="modal-content bg-white">
			...
			<div class="modal-body" style="padding-top: 5px;">
				<ul class="navbar-nav">
					<li class="nav-item" id="result_show_info">
						<a class="nav-link">
						<i class="fa fa-info-circle text-info"></i> Show Info
						</a>
					</li>
					<li class="nav-item" id="result_add">
						<a class=" nav-link active">
						<i class="fa fa-plus-square text-info"></i> Add New Result
						</a>
					</li>
					<li class="nav-item"  id="result_edit">
						<a class="nav-link">
						<i class="fa fa-edit text-info"></i> Edit Result
						</a>
					</li>
					<li class="nav-item" id="result_delete">
						<a class="nav-link">
						<i class="fa fa-trash-alt text-info"></i> Delete Result
						</a>
					</li>
				</ul>
			</div>
			...
			<div class="modal-body" style="padding-top: 5px;">
				<ul class="navbar-nav">
					<li class="nav-item" id="multi_delete">
						<a class="nav-link " > <i class="fa fa-trash-alt text-info"></i> Delete Result(s)</a>
					</li>
					<li class="nav-item" id="multi_share">
						<a class="nav-link " > <i class="fas fa-share text-info"></i> Share Result(s)</a>
					</li>
				</ul>
			</div>
			<!-- /Modal Body -->
