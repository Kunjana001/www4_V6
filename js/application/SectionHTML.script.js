...
// FileName SectionHTML.script.js: Section Javascript file for Cordova project
...
function onBackKeyDown() {
	...
	sectionScript.onClickListBackButton();
...
function onDeviceReady() {
	...
	$(".main-container").prepend( sectionSearchBar );
	$(".main-container").prepend( sectionHeader );

	$("#activity_title").text( "Section" );
	...
	sectionScript.bindFormEventHandlers();

	// Hide Banner Ads
	// hideBanner();

	sectionScript.onListDocumentReady();
}
...
function onFileUploadSuccess() {
	...
	if( mFile != null && mUploadedFiles.length == 0 ) {
		...
		sectionScript.uploadDocuments();
	}
	else {

		sectionScript.onConfirmNetworkSaveData();
	}
}
...
<div class="header" data-theme="a" style="position: fixed;" id="header_id">
	<ul class="header-items">
		...
		<li id="btn_share_page" class="float-right" title="Share Section List">
			<a href="#" class="header-icons-L">
...
var sectionSearchBar = `	<!-- Search Bar-->
<div class="page-container" id="search_bar">
	...
	<div class="form-group" style="margin: -8px;">
		<div class="input-group mb-4">
			...
			<!-- Search Button: sits inside the same input-group as
			     ...
			     Section.script.js/onListDocumentReady(); this button
	...
	<!-- STANDARDIZED TOOLBAR (identical markup on Student/Category/
	     ...
	     click handlers in Section.script.js/onListDocumentReady()
	     keep working. -->
	...
	<div class="modal-dialog modal-danger modal-dialog-centered modal-10" role="document">
		<div class="modal-content bg-white">
			...
			<div class="modal-body" style="padding-top: 5px;">
				<ul class="navbar-nav">
					<li class="nav-item" id="section_show_info">
						<a class="nav-link">
						<i class="fa fa-info-circle text-info"></i> Show Info
						</a>
					</li>
					<li class="nav-item" id="section_add">
						<a class=" nav-link active">
						<i class="fa fa-plus-square text-info"></i> Add New Section
						</a>
					</li>
					<li class="nav-item"  id="section_edit">
						<a class="nav-link">
						<i class="fa fa-edit text-info"></i> Edit Section
						</a>
					</li>
					<li class="nav-item" id="section_delete">
						<a class="nav-link">
						<i class="fa fa-trash-alt text-info"></i> Delete Section
						</a>
					</li>
				</ul>
			</div>
			...
			<div class="modal-body" style="padding-top: 5px;">
				<ul class="navbar-nav">
					<li class="nav-item" id="multi_delete">
						<a class="nav-link " > <i class="fa fa-trash-alt text-info"></i> Delete Section(s)</a>
					</li>
					<li class="nav-item" id="multi_share">
						<a class="nav-link " > <i class="fas fa-share text-info"></i> Share Section(s)</a>
					</li>
				</ul>
			</div>
			<!-- /Modal Body -->
